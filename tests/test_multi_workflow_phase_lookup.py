...
class TestGetPhaseForTaskWithWorkflowId:
    ...
    def test_get_phase_for_task_uses_explicit_workflow_id(
        self, phase_manager_with_two_workflows
    ):
    ...
    def test_phase_belongs_to_correct_workflow(self, phase_manager_with_two_workflows):
        ...
        phase_id = manager.get_phase_for_task(
            phase_id=None, order=1, workflow_id=workflow_id_2
        )
    ...
    def test_without_explicit_workflow_id_uses_singleton(
        self, phase_manager_with_two_workflows
    ):
        ...
        phase_id_explicit = manager.get_phase_for_task(
            phase_id=None, order=1, workflow_id=workflow_id_1
        )
...
class TestMultipleWorkflowPhaseSeparation:
    ...
    def test_phase_names_match_workflow_type(self, manager_with_workflows):
        ...
        try:
            ...
            assert phase_a1.name == "A-Phase-1", (
                f"Expected 'A-Phase-1', got '{phase_a1.name}'"
            )
            ...
            assert phase_b1.name == "B-Phase-1", (
                f"Expected 'B-Phase-1', got '{phase_b1.name}'"
            )
        finally:
            session.close()
...
class TestWorkflowIdSingletonBehavior:
    ...
    def phase_manager(self, db_manager):
        ...
        manager.register_definition(
            ...
            phases_config=[
                {"order": 1, "name": "Phase 1", "description": "Test phase"}
            ],
        )
        return manager
