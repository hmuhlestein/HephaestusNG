...
def _setup_restart_prereqs(db_manager, *, cli_type="pi", cli_model="some-model", agent_type="phase", workflow_id="wf-r", task_id="task-r", phase_id="phase-r"):
...
class TestRestartModelResolution:
    ...
    async def test_uses_agent_cli_model_when_set(self, restart_agent_manager, db_manager):
        ...
        with patch("src.agents.launch_pipeline.get_cli_agent") as mock_get_cli, patch("src.agents.launch_pipeline.asyncio.sleep", new_callable=AsyncMock):
            mock_cli = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_falls_back_to_global_when_agent_cli_model_empty(self, restart_agent_manager, db_manager, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_falls_back_to_cli_default_when_both_agent_model_and_global_absent(self, restart_agent_manager, db_manager):
...
class TestRestartSessionId:
    ...
    async def test_session_id_populated_for_phase_agent(self, restart_agent_manager, db_manager):
        ...
        _, call_kwargs = mock_cli.get_launch_command.call_args
        assert call_kwargs["session_id"] != "", "phase agent should get a non-empty session_id from launch_params"
    ...
    @pytest.mark.asyncio
    async def test_arbitration_agent_gets_session_id_in_restart(self, restart_agent_manager, db_manager):
        ...
        try:
            ...
            raw.execute(sa_text("CREATE TABLE agents_backup AS SELECT * FROM agents"))
            ...
            raw.execute(sa_text("UPDATE agents SET agent_type='arbitration' WHERE id=:id"), {"id": agent_id})
        ...
        with (
            ...
            patch.object(
                restart_agent_manager.branch_manager,
                "get_agent_branch_path",
                return_value=None,
            ),
        ):
            mock_cli = MagicMock()
        ...
        _, call_kwargs = mock_cli.get_launch_command.call_args
        assert call_kwargs["session_id"] != "", "arbitration agents currently get session_id in restart_agent (unlike create_agent_for_task) — this is the Phase 3 mismatch"
...
class TestRestartPromptDelivery:
    ...
    @pytest.mark.asyncio
    async def test_delivers_prompt_via_send_initial_prompt_with_retry(self, restart_agent_manager, db_manager):
    ...
    @pytest.mark.asyncio
    async def test_delivers_full_message_when_no_worktree(self, restart_agent_manager, db_manager):
...
class TestRestartWorktreeResolution:
    ...
    @pytest.mark.asyncio
    async def test_uses_workflow_working_directory_when_present(self, restart_agent_manager, db_manager, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_silent_none_when_no_workflow_wd_and_no_agent_branch(self, restart_agent_manager, db_manager):
        ...
        _, call_kwargs = mock_cli.get_launch_command.call_args
        assert call_kwargs["working_directory"] is None, "Both workflow working_directory and agent branch path absent should produce restart_wd=None without raising"
    ...
    async def test_session_name_has_r_suffix(self, restart_agent_manager, db_manager, tmp_path):
        ...
        session_name_arg = new_session_call.kwargs.get("session_name") or (new_session_call.args[0] if new_session_call.args else "")
        assert "_r" in session_name_arg, f"Restart session name should contain _r suffix, got: {session_name_arg}"
...
class TestRestartGapClosings:
    ...
    @pytest.mark.asyncio
    async def test_restart_calls_termination_race_check_after_sleep(self, restart_agent_manager, db_manager, tmp_path):
        ...
        with patch("src.agents.launch_pipeline.get_cli_agent") as mock_get_cli, patch("src.agents.launch_pipeline.asyncio.sleep", new_callable=AsyncMock):
            ...
            restart_agent_manager._launch._check_termination_race = AsyncMock(return_value=mock_term_info)
    ...
    @pytest.mark.asyncio
    async def test_restart_aborts_when_launch_detected_as_failed(self, restart_agent_manager, db_manager, tmp_path):
        ...
        with patch("src.agents.launch_pipeline.get_cli_agent") as mock_get_cli, patch("src.agents.launch_pipeline.asyncio.sleep", new_callable=AsyncMock):
            ...
            restart_agent_manager._launch._detect_launch_failure = MagicMock(side_effect=Exception("CLI failed to start"))
    ...
    @pytest.mark.asyncio
    async def test_restart_uses_active_readiness_detection_not_flat_sleep(self, restart_agent_manager, db_manager, tmp_path):
