...
def test_qa_pass_continues(strict_spec):
    score, meta = S.score_qa(
        {"failed_tests": 0, "passed_tests": 10, "total_tests": 10,
         "critical_issues": 0, "requirements_total": 5, "requirements_met": 5,
         "agent_score": 1.0}, strict_spec)
...
def test_qa_subjective_blend_below_perfect(strict_spec):
    ...
    score, _ = S.score_qa(
        {"failed_tests": 0, "passed_tests": 1, "total_tests": 1,
         "critical_issues": 0, "agent_score": 0.0}, strict_spec)
    assert score == pytest.approx(0.7)


def test_qa_failed_tests_goto_development(strict_spec):
    score, meta = S.score_qa(
        {"failed_tests": 2, "passed_tests": 8, "total_tests": 10,
         "critical_issues": 0, "agent_score": 1.0}, strict_spec)
...
def test_qa_critical_issue_goto_architecture(strict_spec):
    score, meta = S.score_qa(
        {"failed_tests": 0, "passed_tests": 10, "total_tests": 10,
         "critical_issues": 1, "agent_score": 1.0}, strict_spec)
...
def test_qa_low_pass_rate_goto_development(strict_spec):
    score, _ = S.score_qa(
        {"passed_tests": 7, "total_tests": 10, "failed_tests": 3,
         "critical_issues": 0}, strict_spec)
...
def test_relaxed_spec_allows_some_failures():
    relaxed = {"max_failed_tests": 5, "max_critical_issues": 0,
               "required_pass_rate": 50, "min_requirements_met_rate": 50}
    score, meta = S.score_qa(
        {"failed_tests": 3, "passed_tests": 7, "total_tests": 10,
         "critical_issues": 0, "agent_score": 1.0}, relaxed)
...
def test_product_pass_continues():
    score, meta = S.score_product_validation(
        {"verdict": "PASS", "unmet_requirements": [], "agent_score": 1.0}, S.DEFAULT_SPEC)
    assert score >= 0.7
    assert meta["band"] == "pass"
...
def test_build_phase_output_reads_docs_dir(tmp_path):
    ...
    (docs / "qa_result.json").write_text(json.dumps(
        {"failed_tests": 0, "passed_tests": 5, "total_tests": 5, "critical_issues": 0,
         "agent_score": 1.0}))
...
def test_build_phase_output_root_fallback(tmp_path):
    (tmp_path / "product_validation.json").write_text(json.dumps(
        {"verdict": "NEEDS_WORK", "unmet_requirements": ["x"]}))
