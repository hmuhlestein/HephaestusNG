...
from src.core.database import DatabaseManager, Agent, Task, Phase, PhaseExecution, Workflow
...
class TestListActiveExecutions:
    ...
    def test_with_workflows(self, phase_manager, mock_db):
        session = Mock()
        wf1 = Mock(id="wf-1")
        session.query.return_value.options.return_value.order_by.return_value.all.return_value = [wf1]
        mock_db.get_session.return_value = session
...
class TestCheckPhaseCompletion:
    def test_completed(self, phase_manager, mock_db):
        ...
        task_query = Mock()
        task_query.filter_by.return_value.filter.return_value.count.return_value = 0  # incomplete = 0
        task_query.filter_by.return_value.count.return_value = 2  # completed = 2
        # Return phase for Phase query, task_query for Task query
        session.query.side_effect = [Mock(filter_by=Mock(return_value=Mock(first=Mock(return_value=phase)))), task_query, task_query]
        mock_db.get_session.return_value = session
    ...
    def test_incomplete(self, phase_manager, mock_db):
        ...
        phase = Mock(id="p1")
        task_query = Mock()
        task_query.filter_by.return_value.filter.return_value.count.return_value = 1  # incomplete = 1
        session.query.side_effect = [Mock(filter_by=Mock(return_value=Mock(first=Mock(return_value=phase)))), task_query]
        mock_db.get_session.return_value = session
