...
async def test_agent_manager_with_worktree_integration(agent_manager, test_db):
    ...
    agent = await agent_manager.create_agent_for_task(task=task, enriched_data={}, memories=[], project_context="Test context")
    session.close()
...
async def test_parent_child_agent_integration(agent_manager, test_db):
    ...
    # Create parent agent
    parent_agent = await agent_manager.create_agent_for_task(task=parent_task, enriched_data={}, memories=[], project_context="Test context")
    session.close()
...
def test_child_merge_with_active_parent_worktree(worktree_manager, test_db):
    ...
    session = test_db.get_session()
    parent_agent = Agent(id=parent_id, system_prompt="Parent agent", status="working", cli_type="test")
    ...
    session = test_db.get_session()
    child_agent = Agent(id=child_id, system_prompt="Child agent", status="working", cli_type="test")
...
def test_detached_head_after_child_merge(worktree_manager, test_db):
    ...
    session = test_db.get_session()
    child1_agent = Agent(id=child1_id, system_prompt="Child 1 agent", status="working", cli_type="test")
    ...
    session = test_db.get_session()
    child2_agent = Agent(id=child2_id, system_prompt="Child 2 agent", status="working", cli_type="test")
