...
class TestConductor:
    ...
    async def test_analyze_system_state_with_duplicates(self, conductor):
        ...
        assert (
            result["decisions"][0]["type"] == SystemDecision.TERMINATE_DUPLICATE.value
        )
    ...
    async def test_analyze_system_state_low_coherence(self, conductor):
        ...
        escalation_decisions = [
            d for d in result["decisions"] if d["type"] == SystemDecision.ESCALATE.value
        ]
    ...
    @pytest.mark.asyncio
    async def test_execute_duplicate_termination(
        self, conductor, mock_agent_manager, mock_db_manager
    ):
    ...
    async def test_generate_detailed_report(self, conductor):
        ...
        analysis = {
            ...
            "coherence": {"score": 0.8, "issues": ["Minor alignment issues"]},
            "duplicates": [
                {
                    "agent1": "agent-1",
                    "agent2": "agent-2",
