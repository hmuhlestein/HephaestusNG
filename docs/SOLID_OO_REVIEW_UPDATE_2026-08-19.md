...
| 1.12 | Ad hoc `DatabaseManager()`/`WorktreeManager()` construction | **PARTIAL, re-verified 2026-08-21** | This row overstated current scope. Repo-wide grep for ad hoc construction in `src/mcp/` finds only 2 sites now (`queue_routes.py:565`, `control_routes.py:575`) — `feature_routes.py` and `frontend/phase_routes.py` have zero such constructions, contrary to what this row claimed |
