...
| 1.10 | Task/agent serialization duplicated 6× | **PARTIAL, 2026-08-21** | This row's "~34 sites" count was stale/imprecise -- the real count is roughly a dozen, groupable into 3 genuine shapes (summary/detail/minimal-embedded). No new TaskSerializer/AgentSerializer class introduced: `resolve_task_phase()`/`_serialize_agent()` already exist as partial serializer extractions and are correctly adopted at most sites, so the duplication itself wasn't the live problem -- inconsistent adoption of those existing helpers was. Fixed the 5 concrete bugs surveying it found: `get_task()` never resolved phase at all (always null); `get_task_progress()`'s single-task branch bypassed `resolve_task_phase` (raw unscoped `Phase.filter_by`); the MCP task-status tool had the same bug plus a missing `"Z"` UTC suffix; `mcp_protocol.py`'s `get_resource()` also missing `"Z"`; `get_phase_agents()` read `agent.created_at` under the key `"started_at"` (frontend renders it labeled "Started:") instead of `agent.launched_at`, the field that actually means that. None of the 4 call sites had prior test coverage; 5 new tests added against a real `DatabaseManager`. Full class-level consolidation into shared serializers remains undone -- the embedded-shape sites (`get_task_full_details`'s 4 inline mini-dicts) don't cleanly reduce to just 2 dict-builders and would need a parameterized projection, a separate, larger scoping decision |
| 1.11 | Repair/rerun orchestration in API layer | **PARTIAL, re-verified 2026-08-21** | This row's technical claim ("still constructs its own `DatabaseManager`/`WorktreeManager`", "reimplemented a third time") is stale -- `autopilot_api.py` no longer exists; `rerun_design`/`repair_design`/`spawn_repair_review_agent`/`_run_repair` now live in `src/mcp/autopilot/queue_routes.py`, rewritten since the original review (evidenced by the file's own dense trail of "Observed live" incident-fix comments, apparently from the multi-project-concurrency work). Zero `DatabaseManager` construction remains anywhere in the file -- everything goes through `get_db()` (which itself hits `DatabaseManager`'s path-keyed engine cache, confirmed by reading its `__init__`, so repeated calls share one engine/pool, not several) or `get_app_state().db_manager`. The one `WorktreeManager(db_manager)` construction (`:584`) is the same deliberately-fresh-instance pattern §1.12 already verified and justified (`.reload()` would race a shared instance). Agent creation isn't reimplemented either: `spawn_repair_review_agent` calls the real `/api/create_agent_for_task` endpoint via `api_post` (a genuine HTTP call, confirmed by reading `engine_client.py`), and `rerun_design` starts/stops pipelines through the same in-process `AutopilotService` singleton the play/pause button drives, not a spawned subprocess -- the file's own comments say so explicitly. **What remains true**: these 4 functions (~670 lines combined) are still substantive business logic sitting in the API layer rather than `src/autopilot/`, per the proposed `RepairService` extraction. Deliberately not extracted this pass: this is the single most heavily incident-scarred code found in this whole review (dozens of dated "Observed live" production-incident comments), and `queue_routes.py` has been under continuous concurrent editing by this repo's own autopilot process throughout this review session -- the combination of high blast-radius-if-subtly-wrong and high chance of a concurrent-edit collision, for a purely organizational move with no live bug driving it, is worse odds than every other finding this session actually fixed. A future pass should re-scope this as its own deliberate decision, not fold it into a gap check |
