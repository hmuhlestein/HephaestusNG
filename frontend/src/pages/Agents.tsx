      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">{agent.agent_type === 'orchestrator' ? 'orchestrator' : agent.cli_type}{agent.cli_model ? ` / ${agent.cli_model}` : ''}</p>
            {agent.workflow && (
              <p className="text-xs text-violet-600 dark:text-violet-400 mt-0.5">
                ...
                <span className={`ml-1 px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                  agent.workflow.status === 'active' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
                  agent.workflow.status === 'completed' ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' :
              ...
              <div className="flex items-center space-x-4 text-xs text-gray-600 dark:text-gray-400">
                ...
                <div className="flex items-center">
                  ...
                  {agent.current_task.status === 'in_progress' && (
                    <span className="ml-1 text-green-600 dark:text-green-400">• running</span>
                  )}
                </div>
                <div className={`px-1.5 py-0.5 rounded text-xs ${
                  agent.current_task.priority === 'high' ? 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300' :
                  agent.current_task.priority === 'medium' ? 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300' :
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              !showAll ? 'bg-violet-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
            }`}
          >
            Active
          </button>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4">
          ...
          <p className="text-2xl font-bold text-gray-500 dark:text-gray-400">{agents.filter(a => a.status === 'terminated').length}</p>
        </div>
      </div>
      ...
      {data && data.pages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50"
          >
            Previous
          </button>
