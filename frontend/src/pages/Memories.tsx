...
const MemoryItem: React.FC<{ memory: Memory; isNew?: boolean }> = ({ memory, isNew }) => {
  ...
  return (
    <motion.div
      initial={isNew ? { opacity: 0, x: -20, backgroundColor: highlightColor } : false}
      animate={{ opacity: 1, x: 0, backgroundColor: isNew ? highlightColor : restColor }}
      transition={{ duration: 0.5, backgroundColor: { duration: 2 } }}
      className="rounded-lg shadow-md p-4 mb-4 cursor-pointer hover:shadow-lg transition-shadow"
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className={`text-sm text-gray-800 dark:text-gray-200 ${!isExpanded ? 'line-clamp-2' : ''}`}>
                {memory.content}
              </p>
              {isExpanded && (
                <motion.div
                ...
                >
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      ...
                      <p className="text-gray-800 dark:text-gray-200">{memory.memory_type.replace('_', ' ')}</p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-400 font-medium mb-2">Agent</p>
                      <ClickableAgentCard
                    ...
                    {memory.related_task_id && (
                      <div>
                        <p className="text-gray-600 dark:text-gray-400 font-medium mb-2">Related Task</p>
                        <ClickableTaskCard
                    ...
                    {memory.tags && memory.tags.length > 0 && (
                      <div>
                        <p className="text-gray-600 dark:text-gray-400 font-medium">Tags</p>
                    ...
                    {memory.related_files && memory.related_files.length > 0 && (
                      <div className="col-span-2">
                        <p className="text-gray-600 dark:text-gray-400 font-medium">Related Files</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {memory.related_files.map((file, index) => (
                            <span
                              key={index}
                              className="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded text-xs font-mono"
                            >
                              {file}
                            </span>
                          ))}
  ...
  if (error) {
    return (
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
        <p className="text-red-600 dark:text-red-400">Failed to load memories</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Memories</h1>
      ...
      <div className="grid grid-cols-6 gap-4">
        {['error_fix', 'discovery', 'decision', 'learning', 'warning', 'codebase_knowledge'].map(
          (type) => {
            ...
            return (
              <div key={type} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 text-center">
                <MemoryTypeIcon type={type} />
                <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{type.replace('_', ' ')}</p>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <select
              ...
              className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary"
            >
