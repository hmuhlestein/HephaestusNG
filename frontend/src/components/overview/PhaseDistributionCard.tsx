...
const getPhaseHue = (order: number, totalPhases: number): number => (order - 1) * (360 / totalPhases);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {phases.map((phase) => {
              const phaseProgress = phase.total_tasks > 0
                ...
                : 0;
              const hue = getPhaseHue(phase.order, phases.length);
              const phaseColor = `hsl(${hue}, 70%, 50%)`;
              const phaseTint = `hsla(${hue}, 70%, 50%, ${isDark ? 0.18 : 0.08})`;
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: phase.active_agents > 0 ? phaseColor : (isDark ? '#374151' : '#e5e7eb'),
                    backgroundColor: phase.active_agents > 0 ? phaseTint : (isDark ? '#1f2937' : 'white')
                  }}
                >
                  ...
                  <h4 className="font-medium text-sm mb-2 line-clamp-1 text-gray-900 dark:text-gray-100">{phase.name}</h4>
