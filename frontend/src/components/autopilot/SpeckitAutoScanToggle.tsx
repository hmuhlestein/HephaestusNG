...
const SpeckitAutoScanToggle: React.FC<SpeckitAutoScanToggleProps> = ({ projectId, enabled: enabledProp }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (_, next) => {
      toast.success(next ? 'Auto scan for specs on — ready features build automatically' : 'Auto scan for specs off');
    },
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-1.5 text-sm font-medium transition-colors duration-150 ${
        ...
        {enabled ? <><Search className="w-4 h-4" />Auto scan for specs</> : <><Ban className="w-4 h-4" />Auto scan off</>}
      </div>
      <button
        disabled={disabled}
        onClick={() => mutation.mutate(!enabled)}
        aria-pressed={enabled}
        aria-label="Toggle auto scan for specs"
