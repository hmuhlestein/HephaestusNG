...
- **HIPAA/PII compliance is a dedicated gate, not a hoped-for side effect.** `hipaa_compliance` runs right after security_review, on already-hardened code, and checks the regulatory angle specifically: encryption at rest/in transit, minimum-necessary access control, audit logging, retention/deletion support, and third-party exposure for every PHI/PII field the feature touches. PII-awareness starts upstream too — `product_requirements` identifies and lists PHI/PII fields, `architecture_design` requires an explicit encryption/access-control/redaction decision per field, and `development` follows through on it — so the gate is checking decisions that were actually made, not discovering the problem cold. See [Pipeline Phases](docs/autopilot.md#pipeline-phases).
...
- **Multiple projects in parallel, each with its own child repos.** Run several projects concurrently under a configurable concurrency cap (default 2), not just one at a time — see [Projects & Concurrent Execution](docs/autopilot.md#projects--concurrent-execution). Within a single project, add and label child repos alongside the primary one — a feature scoped to a specific child repo runs its whole pipeline (worktree, branch, merge) against that repo, not the project's primary. See [Multi-Repo Projects](docs/multi-repo-projects.md).
- **Builds from Spec Kit, not just hand-written designs.** Point `--feature` at a [Spec Kit](https://github.com/github/spec-kit) `specs/<NNN>-<name>/` directory your team already authored (`spec.md`/`plan.md`/`tasks.md`) and Autopilot builds from it directly — Hephaestus only reads what Spec Kit already wrote, never runs `specify init` itself. See [Spec Kit Support](docs/speckit.md).
...
- **Ship unattended, or gate on human approval.** By default a feature merges to `main` on its own once every phase passes. Flip a project to **Review Mode** and it instead pushes the branch, opens a PR, and iterates on real CI/review feedback — follow-up commits on the *same* PR, not a fresh one each retry — then pauses for an explicit approve/request-changes before merging. See [Full Autopilot vs. Review Mode](docs/autopilot.md#full-autopilot-vs-review-mode).

**Full reference — every phase, cost tracking and budget enforcement, project selection and concurrent execution, Review Mode vs. full autopilot, and the worktree/data model: [docs/autopilot.md](docs/autopilot.md).**
...
- **Kiro** (default), **Claude Code**, **Codex**, **OpenCode**, **Droid**, **pi**, or **Swarm** - CLI AI tool that agents run inside
