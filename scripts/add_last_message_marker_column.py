...
def add_column():
    ...
    with engine.connect() as conn:
        try:
            ...
            conn.commit()
            print(
                "✅ Successfully added last_claude_message_marker column to guardian_analyses table"
            )
            print(f"   Database: {config.database_path}")
        except Exception as e:
            if "duplicate column name" in str(e).lower():
                print(
                    "⚠️  Column last_claude_message_marker already exists in guardian_analyses table"
                )
            else:
