...
class LaunchPipeline:
    ...
    def _resolve_project_base_dir(self, workflow_id: Optional[str], repo_id: Optional[str] = None) -> Optional[Path]:
        """Resolve workflow_id's project base_dir via Workflow.project_id ->
        AutopilotProject.base_dir. When repo_id is provided, resolves to
        that specific ProjectRepo.path instead (REQ-07/08).
        ...
        try:
            ...
            try:
                ...
                if not wf or not wf.project_id:
                    return None
                # REQ-07: Resolve via repo_id when provided
                if repo_id:
                    try:
    ...
    def _scoped_worktree_manager(self, workflow_id: Optional[str], repo_id: Optional[str] = None) -> WorktreeManager:
        ...
        Args:
            workflow_id: the workflow to scope to.
            repo_id: optional specific ProjectRepo to use (REQ-07/08).
                When provided, resolves to that repo's path instead of
                the project's base_dir.
        """
        base_dir = self._resolve_project_base_dir(workflow_id, repo_id)
        if base_dir is None:
    ...
    async def create_agent_for_task(
        ...
        wt_mgr = self._scoped_worktree_manager(task.workflow_id, task.repo_id)
