    ...
    def _terminate_agent_sync(self, agent_id: str) -> None:
        ...
        try:
            ...
            if agent.pending_message_sent_at:
                ...
                session.commit()
                if remaining > 0:
                    logger.info(f"[TERMINATE] Agent {agent_id[:8]} has a message sent {elapsed:.0f}s ago -- waiting {remaining:.0f}s more before terminating")
            ...
            try:
                wip = self.branch_manager.commit_changes(agent_id, f"[WIP] Auto-saved on terminate of agent {agent_id[:8]}")
                if isinstance(wip, dict) and wip.get("files_changed"):
                    logger.info(f"[TERMINATE] Saved WIP for agent {agent_id[:8]}: {wip['commit_sha'][:8]} ({wip['files_changed']} file(s))")
                ...
                logger.debug(f"[TERMINATE] WIP commit via agent-branch path skipped for {agent_id[:8]}: {e} -- trying shared-worktree fallback")
                            ...
                            if tmux_sess.name == agent.tmux_session_name:
                                ...
                                if _task and _task.phase_id and _task.workflow_id:
                                    try:
                                        ...
                                        _wf = session.query(_Workflow).filter_by(id=_task.workflow_id).first()
                                        if _phase and _wf and _wf.working_directory:
                                            tmux_dir = Path(_wf.working_directory) / CONTEXT_DIR_NAME / "tmux"
                                            tmux_dir.mkdir(parents=True, exist_ok=True)
                                            log_file = tmux_dir / f"{_phase.name}_{agent_id[:8]}.log"

                                            clean_scrollback = full_scrollback
                                            log_file.write_text(clean_scrollback)
                                            logger.info(f"[TMUX-LOG] Final capture for {_phase.name}/{agent_id[:8]}: {len(clean_scrollback)} chars → {log_file.name}")
                                    except Exception as _te:
                                        logger.debug(f"[TMUX-LOG] Final capture write failed: {_te}")
                                break
                except Exception as e:
    ...
    def _commit_wip_in_shared_worktree(self, agent_id: str, task_id: Optional[str]) -> None:
        ...
        try:
            ...
            try:
                ...
                if task and task.workflow_id:
                    ...
                    if wf:
                        ...
                        # REQ-16: the workflow's single working_directory is
                        # only guaranteed to be the RIGHT repo for a task
                        # whose own repo_id (if any) agrees with it -- a
                        # multi-repo project's task could be scoped to a
                        # sibling repo the workflow's worktree never
                        # touched. Refuse to commit into the wrong repo
                        # rather than silently doing it.
                        if task.repo_id and wf.project_id:
                            from src.core.repo_resolution import resolve_repo

                            repo = resolve_repo(session, wf.project_id, task.repo_id)
                            if repo and working_directory and Path(repo.path).resolve() != Path(working_directory).resolve():
                                logger.warning(
                                    f"[TERMINATE] Task {task_id[:8]}'s repo_id resolves to "
                                    f"{repo.path}, not the workflow's working_directory "
                                    f"{working_directory} -- skipping shared-worktree WIP "
                                    "commit rather than risk committing into the wrong repo"
                                )
                                return
            finally:
                session.close()
            ...
            if not repo.is_dirty() and not repo.untracked_files:
                return
            repo.git.commit("-m", f"[Agent {agent_id}] [WIP] Auto-saved on terminate", "--no-verify")
            logger.info(f"[TERMINATE] Saved WIP for shared-worktree agent {agent_id[:8]} in {working_directory}")
        except Exception as e:
            logger.warning(f"[TERMINATE] Shared-worktree WIP commit also failed for {agent_id[:8]}: {e}")
