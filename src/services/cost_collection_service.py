...
def _discover_claude_session_file(session_id: str, cwd: str) -> Optional[Path]:
    """Discover a Claude Code session file by session_id and cwd.

    Claude Code session files are stored in ~/.claude/projects/<sanitized_cwd>/
    as {uuid}.jsonl, where uuid = uuid5(NAMESPACE_URL, session_id). Split out
    of collect_task_cost so delete_feature's session cleanup (feature_routes.py)
    can reuse the same discovery logic instead of duplicating it.
    """
    # SECURITY: Sanitize cwd to prevent path traversal
    if ".." in cwd or "~" in cwd:
    ...
    # Claude Code files are named {uuid}.jsonl where uuid = uuid5(NAMESPACE_URL, session_id)
...
def collect_task_cost(task_id: str) -> None:
    ...
    with get_db() as db:
        ...
        if cli_type == "pi":
            ...
            cwd = _get_agent_cwd(db, agent, task)
            if cwd:
                session_file = _discover_claude_session_file(session_id, cwd)
...
def _extract_session_id(agent: Any, task: Any) -> Optional[str]:
    ...
    src/autopilot/phases.py from (project_id, design_slug, workflow_id,
    phase_name, model) and passed to pi via --session-id.  It is NOT
    stored on the agent or task rows, so we reconstruct it from the
    workflow's launch_params, the task's own workflow_id, and the task's
    phase.
    """
    ...
    try:
        with get_db() as db:
            ...
            return get_session_id(project_id, design_slug, phase_name, model=model, workflow_id=task.workflow_id)
    except Exception as e:
