...
```python
heph_search_memory(query="[workflow goal keywords + stuck issue]", limit=10)
```
...
- [ ] Used `heph_search_memory` to search memories (Step 2)
...
heph_create_task(description="...", phase_id=1, ...)  # From available phases
heph_create_task(description="...", phase_id=2, ...)  # From available phases
```

**Scenario 2: Tests done but no evidence → Phase 4 (Documentation)**
```python
# Step 5 mapping: Evidence gap → Phase 4 (Documentation)
heph_create_task(description="Compile evidence.md", phase_id=4, ...)
heph_create_task(description="Submit with submit_result()", phase_id=4, ...)
```

**Scenario 3: Multiple gaps**
```python
# Step 5 mapping: Design gap → Phase 1, Code gap → Phase 2, Test gap → Phase 3
heph_create_task(description="...", phase_id=1, ...)  # Must be in available list
heph_create_task(description="...", phase_id=2, ...)  # Must be in available list
heph_create_task(description="...", phase_id=3, ...)  # Must be in available list
```
...
1. Understand Goal
2. Search Memory (heph_search_memory)
3. Analyze State
4. Identify Gap
