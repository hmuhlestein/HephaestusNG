    ...
    try:
        ...
        with get_db() as db:
            ...
            try:
                ...
                logger.warning(f"[REPO-RESOLUTION] Failed to resolve repo for commit {commit_sha}: {e} -- returning None")
                return None
    except Exception as e:
        logger.warning(f"[REPO-RESOLUTION] Unexpected error resolving repo for commit {commit_sha}: {e}")
        return None
...
def _resolve_repo_id_and_label_for_commit(commit_sha: str) -> "tuple[Optional[str], Optional[str]]":
    ...
    try:
        ...
        logger.warning(f"[REPO-RESOLUTION] Failed to resolve repo_id/label for commit {commit_sha}: {e}", exc_info=True)
        return None, None
...
def _gather_clarification_context(ticket_id: str):
    ...
    from src.core.database import Task, Ticket
