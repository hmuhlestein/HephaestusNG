...
def heal_orphaned_agent_branches(logger: "OrchestratorLogger") -> int:
    ...
    try:
        with db.session_scope() as session:
            # REQ-16: Iterate ProjectRepos instead of AutopilotProjects
            repo_paths = {repo.path for repo in session.query(ProjectRepo).all() if repo.path and Path(repo.path).is_dir()}

        for repo_path in repo_paths:
            try:
                healed += _heal_orphaned_branches_for_project(Path(repo_path), cfg, logger)
            except Exception as e:
                logger.warning(f"[BRANCH-HEAL] Failed to scan {repo_path}: {e}")
    except Exception as e:
