...
def run_feature_pipelines(
    ...
    for group in execution_groups:
        ...
        if len(features_to_run) == 1:
            ...
            feature_project_path = _resolve_feature_project_path(project_path, project_id, design_entry.db_id, feature_key, logger)
            ...
            with ThreadPoolExecutor(max_workers=MAX_PARALLEL_FEATURES) as executor:
                future_to_feature = {
                    executor.submit(
                        ...
                        _resolve_feature_project_path(project_path, project_id, design_entry.db_id, feat.get("id", "unknown"), logger),
                        logger,
                        state,
                        max_iterations,
                        project_id,
