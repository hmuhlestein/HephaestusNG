"""
Autopilot pipeline execution flow.

The actual run-the-pipeline logic for the orchestrator package: watches the
design queue, runs phase0, runs each feature's workflow, and drives the
continuous top-level loop. Moved out of orchestrator/__init__.py (SOLID
review: a package __init__ should be a re-export surface, not host 3000+
        ...
        with get_db() as db:
            ...
            if feat and feat.workflow_id:
                ...
                if wf and wf.paused_by != "review" and wf.status != "failed":
                    # NOTE: do NOT gate on wf.status == "completed" here to
                    # avoid re-pausing an already-reviewed workflow -- the
                    # feat.review_status == "approved" check above already
                    # covers that, deliberately, via a value only the
                    # approval endpoint ever sets. wf.status can be flipped
                    # to "completed" by an unrelated, review-mode-blind
                    # writer (derive_workflow_status(write_back=True), polled
                    # every ~10s by /design-status) in the narrow window
                    # after the last phase finishes but before this function
                    # (or _complete_workflow's own pause branch) gets to
...
def _run_one_feature(
    ...
    with get_db() as db:
        ...
        if feat_record:
            ...
            # Resolved here (before the "already completed" cleanup branch
            # below) rather than after this whole block, so that branch's own
            # _create_integration_worktree/_cleanup_worktree calls target the
            # feature's real repo too -- moved up after a bug found where the
            # cleanup path used the project's primary path even for a feature
            # bound to a different repo, silently missing its actual worktree.
            if project_id:
