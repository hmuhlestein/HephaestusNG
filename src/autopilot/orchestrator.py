        ...
        if phase_name in GATED_PHASES:
            with get_db() as db:
                ...
                if wf and wf.working_directory and Path(wf.working_directory).exists():
                    phase_output = build_phase_output(phase_name, Path(wf.working_directory))
        ...
        with get_db() as db:
            ...
            if existing:
                ...
                if existing.status == "pending" and not existing.assigned_agent_id:
                    logger.info(f"[PHASE-TASK] {phase_name} has an orphaned pending task {existing.id[:8]} (never dispatched) -- marking failed and creating a fresh one")
