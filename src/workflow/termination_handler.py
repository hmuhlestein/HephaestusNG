...
class WorkflowTerminationHandler:
    ...
    async def terminate_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            agents_terminated = await self._terminate_workflow_agents(
                workflow_id, termination_results
            )
            logger.info(
                f"Terminated {agents_terminated} agents for workflow {workflow_id}"
            )

            # 2. Cancel all pending and in-progress tasks
            tasks_cancelled = await self._cancel_workflow_tasks(
                workflow_id, session, termination_results
            )
            logger.info(f"Cancelled {tasks_cancelled} tasks for workflow {workflow_id}")

            # 3. Clean up workflow resources
            cleanup_actions = await self._cleanup_workflow_resources(
                workflow_id, session, termination_results
            )
            logger.info(
        ...
        try:
            ...
            agents = (
                ...
                .filter(
                    ...
                    Agent.agent_type
                    != "result_validator",  # Don't terminate result validators
                )
                .distinct()
                .all()
            )
            ...
            for agent in agents:
                try:
                    logger.info(
                        f"Terminating agent {agent.id} for workflow {workflow_id}"
                    )
    ...
    async def _cancel_workflow_tasks(
        self, workflow_id: str, session, results: Dict[str, Any]
    ) -> int:
    ...
    async def _cleanup_workflow_resources(
        ...
        try:
            ...
            worktrees = (
                ...
                .join(Agent, AgentBranch.agent_id == Agent.id)
                .join(Task, Agent.id == Task.assigned_agent_id)
                .filter(
                    Task.workflow_id == workflow_id,
            ...
            for execution in phase_executions:
                try:
                    ...
                    logger.error(
                        f"Failed to complete phase execution {execution.id}: {e}"
                    )
                    cleanup_actions.append(
                        {
                            "action": "complete_phase_execution",
    ...
    def get_workflow_termination_status(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "is_terminated": workflow.completed_by_result
